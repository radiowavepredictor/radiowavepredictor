...
from common.function.func import flatten_dict,to_yaml_safe
...
class SaveClass(BaseModel):
    model:Any|None=None
    params:dict = Field(default_factory=dict) # デフォルト値を空のdictにする指定がこれらしい
    ...
    def _save_mlflow(self,save_cfg:SaveConfig,run_id:str|None):
        ...
        with mlflow.start_run(run_id):
            ...
            if artifact_dir.startswith("file:"):
                ...
                if len(artifact_dir)>=3 and artifact_dir[0]=="/" and artifact_dir[2]==":":
    ...
    def save(self,save_cfg:SaveConfig,run_id:str|None = None)->str:
        if save_cfg.use_mlflow:
            ...
            artifacts_dir=save_cfg.save_dir
        ...
        return run_id #type:ignore[arg-type]